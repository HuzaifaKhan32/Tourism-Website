      ...
      {isVisible && (
        ...
        >
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-2xl border border-slate-100 dark:border-white/10 flex items-center gap-3 relative group">
            ...
            {/* Close Button: Visible on Hover on Desktop, Always on Mobile */}
            <button 
              onClick={() => setIsVisible(false)}
              className="absolute -top-2 -right-2 size-6 bg-slate-200 dark:bg-slate-700 hover:bg-red-500 hover:text-white rounded-full text-[12px] flex items-center justify-center transition-all shadow-md md:opacity-0 md:group-hover:opacity-100 opacity-100"
              aria-label="Dismiss"
            >
